...
function registerUserTools(server) {
    server.tool("search_users", "Search users by name. Returns id, name and username (email) — use id as responsible_id for tasks or assignee_id for deals", {
        query: zod_1.z.string().describe("Name to search for (case-insensitive partial match)"),
    }, async (args) => {
