...
Type values: 0=Task, 1=Inbox, 20=Event, 30=Template`, {
        ...
        status: zod_1.z.coerce.number().optional().describe("Status: 1=New (backlog), 3=In Progress, 4=Waiting Review, 5=Done. Use 3 for 'currently working on'."),
        ...
        archive_status: zod_1.z.coerce.number().optional().describe("0=Not archived (default), 10=Archived. Pass 10 to see archived tasks."),
        page: zod_1.z.coerce.number().optional().describe("Page number for pagination"),
    }, async (args) => {
        const { page, ...filters } = args;
        if (filters.archive_status === undefined)
            filters.archive_status = 0;
        const params = {};
        if (page !== undefined)
            params.page = page;
